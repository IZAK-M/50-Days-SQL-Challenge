...
INSERT INTO walmart_eu (invoiceno, stockcode, description, quantity, invoicedate, unitprice, customerid, country)
VALUES
...
-- QUESTIONS:
-- +----------------------------------------------------------------------------------------------------------------------------------+
-- | Trouver l’article le plus vendu pour chaque mois (sans distinguer les années), en se basant sur la facture totale la plus élevée.|
-- | L’article le plus vendu est déterminé selon la formule : unitprice × quantity.                                                   |
-- | Afficher le mois, la description de l’article, ainsi que le montant payé.                                                        |
-- +----------------------------------------------------------------------------------------------------------------------------------+


WITH rang_ventes_mensuel AS
(
SELECT 
	description,
	EXTRACT (MONTH FROM invoicedate) AS mois,
	SUM(unitprice * quantity) AS total_revenue,
	DENSE_RANK() OVER(PARTITION BY EXTRACT (MONTH FROM invoicedate) 
					  ORDER BY SUM(unitprice * quantity) DESC) AS rang
FROM walmart_eu
GROUP BY 1, 2
)
SELECT 
	...
	description,
	total_revenue
FROM rang_ventes_mensuel
WHERE rang = 1;


-- QUESTIONS:
-- +----------------------------------------------------------------------------------------------------------------+
-- |Trouver le client du mois pour chaque mois, c’est-à-dire le client ayant dépensé le montant total le plus élevé |
-- |(calculé avec prix × quantité),en tenant compte de plusieurs achats possibles dans le mois.                     |
-- +----------------------------------------------------------------------------------------------------------------+


WITH meilleur_client_mensuel AS
(
SELECT 
	DISTINCT customerid,
	EXTRACT (MONTH FROM invoicedate) AS mois,
	SUM(unitprice * quantity) OVER(PARTITION BY EXTRACT (MONTH FROM invoicedate), customerid) monthly_client_revenue
FROM walmart_eu
ORDER BY mois, monthly_client_revenue DESC
),
classement 	AS
(
SELECT 
	*,
	ROW_NUMBER() OVER(PARTITION BY mois) AS rang
FROM meilleur_client_mensuel
)
SELECT * 
FROM classement 
WHERE rang = 1;
