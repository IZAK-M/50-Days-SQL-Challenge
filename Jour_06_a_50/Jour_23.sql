...
-- Création de la table amazon_transactions:
...
INSERT INTO amazon_transactions (user_id, item, purchase_date, revenue)
VALUES
...
-- QUESTIONS:
-- +------------------------------------------------------------------------------------------------------------------------------+
-- |Rédige une requête SQL permettant d’identifier les utilisateurs actifs de retour.                                             |
-- |Un utilisateur actif de retour est un utilisateur qui a effectué un deuxième achat dans les 7 jours suivant son premier achat.| 
-- |La requête doit retourner une liste des user_id de ces utilisateurs.                                                          |
-- +------------------------------------------------------------------------------------------------------------------------------+
...
SELECT 
	*,
	ROW_NUMBER() OVER(PARTITION BY user_id ORDER BY purchase_date) AS rang
FROM amazon_transactions
)
SELECT DISTINCT r1.user_id AS active_users
FROM rang_table r1
JOIN rang_table r2 ON r1.user_id = r2.user_id
                  AND r1.rang = 1 AND r2.rang = 2
WHERE (r2.purchase_date - r1.purchase_date) <= 7;

--
-- +-------------------------------------------------------------------------------------------------------------+
-- |Trouver les user_id des utilisateurs qui n'ont effectué aucun achat pendant 7 jours après leur premier achat,|
-- |mais qui ont quand même réalisé un deuxième achat après ces 7 jours.                                         |
-- +-------------------------------------------------------------------------------------------------------------+
